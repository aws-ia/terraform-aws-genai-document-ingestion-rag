locals {
  ...
  graph_ql_url = coalesce(trimspace(var.merged_api_url), aws_appsync_graphql_api.ingestion_graphql_api.uris["GRAPHQL"])
  open_search_api_name = (length(var.opensearch_serverless_collection_endpoint) > 0 ? "aoss"
