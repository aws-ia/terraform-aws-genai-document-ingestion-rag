...
def handler(event,  context: LambdaContext) -> dict:
    ...
    job_id = ingestion_input.get('ingestionjobid', str(custom_job_id))
    ignore_existing = ingestion_input.get("ignore_existing", False)

    logger.info(f"Ingestion Job ID: {job_id}")
    logger.info(f"ingestion_input: {ingestion_input}")
    ...
    embeddings_model = ingestion_input.get('embeddings_model', {'modelId': 'amazon.titan-embed-text-v1'})
